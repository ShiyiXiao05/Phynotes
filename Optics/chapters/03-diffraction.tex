...
其中$\lambda$为光的波长, $\Delta \theta$为中心衍射斑从中心到首个暗纹的半角宽度, $\rho$为狭缝的宽度. 其中$\Delta \theta$越大, 说明衍射效应越强, 因此我们可以简单地说, 狭缝宽度越小, 衍射现象越明显, 中心衍射斑铺展的越开. 上面这个式子我们会在后面给出推导.
...
复数形式为:
\[
\widetilde U(r,t) = \frac{A}{r}e^{i(kr+\varphi_0)}e^{-i\omega t},\qquad U(r,t)=\operatorname{Re}\widetilde U(r,t)
\]
...
为了求解衍射强度的分布, 我们采用如下的方法: 将缝面处的波阵面划分为$N$个与狭缝平行的等宽窄条, 其中$N$非常大. 注意到窄条发出的子波在$P$点的光振动振幅近似相等, 设每条的振幅为$E_s$. 狭缝最边缘的两个窄条$A,B$发出的衍射光到达$P$点时的光程差$AC= a\sin\theta$. 相邻的两个窄条发出的子波在$P$点的光程差为$\frac{a\sin\theta}{N}$, 相位差为$\Delta \varphi =\frac{2\pi}{\lambda}\cdot \frac{a\sin\theta}{N}$.
...
由几何关系:
\begin{align*}
2\beta &= N\Delta\varphi = \frac{2\pi a\sin\theta}{\lambda},
&\beta &= \frac{\pi a\sin\theta}{\lambda},\\
E_p &= 2R\sin\beta,
&E_{\max} &= NE_s = 2R\beta,
&\frac{E_p}{E_{\max}} &= \frac{\sin\beta}{\beta}.
\end{align*}
...
在这之后我们将多次遇见$\frac{\sin x}{x}$型函数, 可以记作$\operatorname{sinc}(x)$. 我们对这函数的一些性质在下面做些阐述.

\insertfig{3-8.png}{$\operatorname{sinc}(x)$图象}{0.65}

\insertfig{3-9.png}{$\operatorname{sinc}^2(x)$图象}{0.7}

(1)最大值(主极大). 当$x=0$,$\operatorname{sinc}(x)=\frac{\sin x}{x}=1$时为最大值. 这在单缝衍射中, 表现为$\theta=0$时, 衍射强度$I(0)=I_0$为最大值, 称其为零级衍射峰, 其位置正是几何光学像点位置. 这个位置也被称为单缝中央主极大光强, 也被称为中央明纹中心.

(2)零点位置. $\operatorname{sinc}(x)$函数存在一系列零点. 当$x=k\pi,k=\pm 1, \pm 2,\dots$时, $\operatorname{sinc}(x)=0$. 这在单缝衍射中, 表现为当:
\[
a\sin\theta = k\lambda , \quad k = \pm 1, \pm 2,\dots
\]
时, 衍射强度$I(\theta)=0$, 出现暗纹. 上式称为单缝衍射零点条件.

(3)次极大. $\operatorname{sinc}^2(x)$在相邻两个零点间存在一个极大值, 这个位置由:$\frac{\d(\operatorname{sinc}(x))}{\d x} = 0 \Rightarrow x = \tan x$这个超越方程给出, 其数值结果可以被列表如下:
...
\hline
\multirow{2}{*}{$|\operatorname{sinc}(x)|$}
& 0.22 & 0.13 & 0.09 \\
\cline{2-4}
& $4.7\%$ & $1.7\%$ & $0.8\%$ \\
\hline
...
二是在理想的有限矩形狭缝、相同入射辐照度和相同成像条件下, 中央峰值强度不同:
\[
\frac{I_{10}}{I_{20}}=\left(\frac{\lambda_2}{\lambda_1}\right)^2\approx 44\%
\]
若比较的是各自归一化后的衍射图样, 两者中央峰值都定义为$I_0$; 上述比值只适用于给定的照明与孔径条件. 同时可以想见, 可能在某处出现红光和蓝光的次极大和暗纹交叠的情况.
...
\]
时, 将会出现缺级. 由$k'=1,2,3\dots$可以依次推出可能缺级的整数级次为$\frac{d}{a},\frac{2d}{a},\frac{3d}{a}\dots$.
\section{光栅衍射}
...
现在我们来考虑光栅衍射时衍射屏上的光强分布. 对于每个单缝, 设其入射的振幅为$E_0$, 则其在$\theta$衍射角方向上的光振幅为$E_{single}=E_0\frac{\sin \beta}{\beta}$,$\beta= \frac{\pi a\sin\theta}{\lambda}$. 相邻缝发出的光在$P$点的相位差为$\Delta \varphi= \frac{2\pi}{\lambda}d\sin\theta$. 因此我们发现这类似于我们在处理单缝衍射的时候的方法, 只不过这里不是无穷多缝的叠加.
...
因此艾里斑的半角宽度为:
\[
\theta_1 \simeq 1.22\frac{\lambda}{D}
\]
...
\[
d\sin\theta = k \lambda \Rightarrow d\cos\theta\d\theta = k \d \lambda \Rightarrow D_\theta =\frac{k}{d \cos\theta}
\]
...
\[
\delta \theta = D_\theta\,\delta \lambda = \frac{k}{d\cos\theta_k}\delta \lambda, \quad \Delta \theta_k = \frac{\lambda}{Nd \cos\theta_k}
\]
两者相等. 前者是我们根据角色散本领推导出的两主极大之间的角差距, 后者是主极大本身的半角宽度, 瑞利判据告诉我们两者相等的时候恰好可以分辨. 于是我们得到:
\[
