...
(3)次极大. $\operatorname{sinc}$函数在相邻两个零点间存在一个极大值, 这个位置由:$\frac{\d(\operatorname{sinc}(x))}{\d x} = 0 \Rightarrow x = \tan x$这个超越方程给出, 其数值结果可以被列表如下:
...
\hline
\multirow{2}{*}{$\operatorname{\operatorname{sinc}}(x)$}
& 0.22 & 0.13 & 0.09 \\
\cline{2-4}
& $4.7\%$ & $1.7\%$ & $0.8\%$ \\
\hline
