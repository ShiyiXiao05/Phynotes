...
这里$\d\Omega=(\hat{\mathbf r}\cdot\d\mathbf S)/r^2$是有向立体角, 对包围点电荷的任意闭合曲面积分均为$4\pi$. 对于一个位于闭合面外的点电荷, 我们会发现对于一条穿过闭合面上某面元的电场线必然从另一个面元穿出, 从而对电通量不产生贡献, 因此其贡献的电通量为0.由场强叠加定理, 我们就可以将电荷分布视作由若干点电荷构成, 从而我们得到了高斯定律.
...
两个互不相连的导体可构成电容器. 当两导体分别带等量异号电荷$\pm q$, 且周围边界条件固定时, 电势差与$q$成正比. 电容取决于导体几何、介质及周围环境; 若外导体构成理想闭合屏蔽壳, 外部电荷不会影响壳内的电势差. 电容定义为:
\[
C=\frac{q}{V_A-V_B}
\]
...
假如电子被当作点电荷, 将会出现静电能的发散困难. 若把库仑能的特征尺度$e^2/(4\pi\varepsilon_0r)$与电子静止能量$m_ec^2$相比较, 可定义电子的经典半径:
\[
...
\[
\oint_L \mathbf{E}_0 \cdot \d \mathbf{l}  = 0
\]
