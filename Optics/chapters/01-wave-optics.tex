...
按照时间尺度衡量, 波可以被分为定态波和脉冲波. 若在观测时间内光源持续且稳定地发光, 则波场中各点均以同一频率作稳定的振荡, 这种波被称为定态波. 而光源在极短时间中发光, 以至于波形局限于一个小区域内时, 这种波被称为脉冲波. 对于可见光, 其周期$T\approx 10^{-15}\mathrm{s}$, 普通光源从微观时间尺度看其一次持续发光时间量级$\tau\approx10^{-8} \mathrm{s}$. 一个长波列中大约含有$10^6$个周期, 这种情况就可以视为定态了. 发光的机制我们在讨论光的时间相干性时还会遇见.

现在我们考虑光波的相关叠加. 设在同一介质中有两列平面电磁波传播, 分别为:
\[
\bm E_1 = \bm{E}_{10} \cos (\omega t - \bm{k}_1 \cdot \bm{r} + \varphi_1), \quad
\bm E_2 = \bm{E}_{20} \cos (\omega t - \bm{k}_2 \cdot \bm{r} + \varphi_2)
\]

叠加后合成的光矢量为:
\[
\bm E = \bm E_1 + \bm E_2 = \bm{E}_{10} \cos (\omega t - \bm{k}_1 \cdot \bm{r} + \varphi_1) + \bm{E}_{20} \cos (\omega t - \bm{k}_2 \cdot \bm{r} + \varphi_2)
\]

注意到能流密度矢量对时间取平均得到波的强度,
\[
I = \langle S \rangle = \frac{1}{2} c \varepsilon_0 n E_0^2 \propto E_0^2\qquad(\mu_r\simeq1)
\]

这样将上式两端对观察时间求平均得到:
\[
I = K\langle |\bm E|^2\rangle = I_1+I_2+2K\langle\bm E_1\cdot\bm E_2\rangle,\qquad K=c\varepsilon_0n\quad(\mu_r\simeq1)
\]
$2K\langle\bm E_1\cdot\bm E_2\rangle$被称为干涉项.

这里我们先不考虑偏振, 认为
\[
\bm{E}_1 \parallel \bm{E}_2
\]
从而, 把传播带来的相位并入$\varphi_1$和$\varphi_2$, 并用积化和差公式, 可得干涉项:
\[
\begin{aligned}
2K\langle\bm E_1\cdot\bm E_2\rangle
&=K E_{10}E_{20}\bigl\langle
\cos[(\omega_1-\omega_2)t+\varphi_1-\varphi_2]
+\cos[(\omega_1+\omega_2)t+\varphi_1+\varphi_2]
\bigr\rangle.
\end{aligned}
\]
我们把光的空间传播带来的相位落后归结到$\varphi_1, \varphi_2$中. 上式中的和频项(后一项)在光学周期上时间平均为零. 差频项(前一项)只有在$\omega_1=\omega_2$且相位差稳定时, 才能在通常的观测时间内保留下来:
\[
2K\langle \bm E_1\cdot\bm E_2\rangle=K E_{10}E_{20}\cos\Delta\varphi(P)=2\sqrt{I_1I_2}\cos\Delta\varphi(P)
\]
...
\[
\Delta \varphi(P) = k(r_2-r_1) = \frac{2\pi}{\lambda}(r_2-r_1) = \frac{2\pi}{\lambda}\delta
\]
其中$\delta$在这里是同一介质中的几何路程差; 若用光程差, 应乘以介质折射率. 注意到:
\[
\Delta \varphi=
\begin{cases}
\pm 2k\pi, & \text{当 } \delta = k\lambda, k \in \mathbb{Z}, \cos(\Delta \varphi)= 1, \text{干涉光强极大} \\
(2k+1)\pi, & \text{当 } \delta = (k+\frac{1}{2})\lambda, k \in \mathbb{Z}, \cos(\Delta \varphi)= -1, \text{干涉光强极小} \\
\end{cases}
\]
考虑介质的折射率不为1时, 根据我们前面对光程的讨论, 我们可以得到:
\[
\Delta \varphi = \frac{2\pi}{\lambda_0} n (r_2-r_1)=\frac{2\pi}{\lambda_n}\delta
\]
其中$\lambda_0$为真空波长, $\lambda_n=\lambda_0/n$为介质中的波长, $\delta=r_2-r_1$为几何路程差. 于是用波程差表达的干涉条件可以被写为:
\[
\begin{cases}
...
若以$A_i=\sqrt{I_i}$表示强度归一化的振幅, 双光束干涉强度公式为:
\[
...
\Delta \varphi = (2k+1)\pi, & k \in \mathbb{Z}, \quad \text{干涉极小}
\end{cases}
\]
